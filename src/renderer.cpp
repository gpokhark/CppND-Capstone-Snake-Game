...
void Renderer::Render(Snake const snake, Snake const snake_p2, SDL_Point const &food) {
  SDL_Rect block;
  ...
  // Render p2 snake's body
  SDL_SetRenderDrawColor(sdl_renderer, 0xFF, 0xFF, 0xFF, 0xFF);
  for (SDL_Point const &point : snake_p2.body) {
    block.x = point.x * block.w;
    block.y = point.y * block.h;
    SDL_RenderFillRect(sdl_renderer, &block);
  }

  // Render p2 snake's head
